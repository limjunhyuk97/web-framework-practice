...
var _default = function _default(targetElement, state, events) {
  ...
  var searchList = state.searchList,
      curFocusedItem = state.curFocusedItem; // 검색한 내역이 없다면 border 지워라

  targetElement.style.border = searchList.length === 0 ? "none" : "1px solid black"; // 검색 결과 보여주는 요소 복사해서 가상 요소 만듬 + 내부 제거

  var newSearchedList = targetElement.cloneNode(true);
  newSearchedList.innerHTML = ""; // 검색 결과들에 대해서 template 사용해서 html요소로 변환 + 요소 부착
...
var _default = function _default(targetElement, state, events) {
  var newApp = targetElement.cloneNode(true); // 일단 VDOM 내부 한번 비워주고

  newApp.innerHTML = ""; // template 없으면 만들고, 있으면 가져다 쓴다.

  newApp.appendChild(getTemplate()); // VDOM에 event 등록
...
_registry.default.add("app", _app.default); // *상태 -> 오직 상태에만 순수하게 의존하여 렌더링 되는 화면 구현
